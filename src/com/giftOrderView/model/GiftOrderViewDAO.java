...
public class GiftOrderViewDAO implements GiftOrderViewDAO_interface {
	...
	private static final String GET_ALL_BY_MEMOTHER_STMT = "SELECT * FROM GIFT_ORDER_VIEW WHERE MEM_NO_OTHER=? ORDER BY GIFT_NO DESC";
	...
	public GiftOrderViewVO getByPrimaryKey(String giftr_no) {
		...
		try {
			...
			giftOrderViewVO.setGiftr_message(rs.getString("giftr_message"));
	...
	public List<GiftOrderViewVO> getReceiveAll(String mem_no_other) {
		...
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement(GET_ALL_BY_MEMOTHER_STMT);
